...
def ViewSignal(Type, amplit, offset, freqStart, freqEnd):
    ...
    if Type == "square":
        # Créer les signaux carrés
        signal_start = amplit * np.sign(np.sin(2 * np.pi * freqStart * Period_start)) + offset
        signal_end = amplit * np.sign(np.sin(2 * np.pi * freqEnd * Period_end)) + offset
