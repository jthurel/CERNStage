...
def ViewSignal(Type, amplit, offset, freqStart, freqEnd, file):
    freqStart=find(file, "FREQ",2)
    freqStart=find(file, "FREQ",2)
    freqStart=find(file, "FREQ",2)
    ...
    if Type == "triangle":
        ...
        signal_start = amplit * (1 - 4 * abs((table_start * freqStart + 0.25) - np.floor(table_start * freqStart + 0.25)) - 1) + offset
        signal_end = amplit * (1 - 4 * abs((table_end * freqEnd + 0.25) - np.floor(table_end * freqEnd + 0.25)) - 1) + offset
