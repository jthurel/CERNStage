...
def Start_Sweep(port):
    ...
    open_result_file_button.config(state="disabled")
...
def Download_Data(fileofinterest):
    global dir_folder, pathfile, result_file_path  # Ajoutez pathfile aux variables globales
    ...
    if fileofinterest:
        ...
        if "15" in data_ready:
            ...
            result_file_path = pathfile

            with open(pathfile, "w", encoding="utf-8") as f:
                f.write(results.replace("\r", ""))

            download_data_result.set("The data has been sent")
            open_result_file_button.config(state="active")

            pathfile_csv = f"{path_to_logs}/{filename}"
            return pathfile_csv
...
def Open_Resultinit():
    global result_file_path  # Ajoutez cette ligne pour accéder à la variable globale

    if result_file_path:
        selected_file_path = result_file_path
        syst_os = platform.system()
        if syst_os == "Linux":
            subprocess.run(["xdg-open", selected_file_path])
        elif syst_os == "Windows":
        ...
        print("Le chemin du fichier résultant est vide.")
def Open_Result():
    global pathfile  # Accédez à pathfile en tant que variable globale

    if pathfile:
        selected_file_path = pathfile  # Utilisez pathfile dans la fonction
        syst_os = platform.system()
        if syst_os == "Linux":
            subprocess.run(["xdg-open", selected_file_path])
        elif syst_os == "Windows":
...
download_data_button.config(state="disabled")
download_data_button.grid(row=7, column=0,columnspan=1, sticky="ew")
...
# Créer le bouton "Open Result File"
Path=[]
open_result_file_button = tk.Button(root, text="Open Result File", command=lambda: Open_Result)
open_result_file_button.config(state="disabled")
open_result_file_button.grid(row=7, column=1,columnspan=1, sticky="ew")
pathfile_result= Path
